...
func (s *Server) Handler() (http.Handler, error) {
	...
	router.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(protected chi.Router) {
			...
			protected.With(permission("apps:submit")).Post("/apps/{app}/submit", s.submitApp)
			protected.With(permission("apps:update")).Get("/apps/{app}/security-check", s.appSecurityCheck)
			protected.With(permission("apps:update")).Post("/apps/{app}/security-check/verify", s.verifyAppSecurityCheck)
...
func (s *Server) adminRoutes(r chi.Router) {
	...
	r.With(permission("settings:read")).Get("/admin/security-check", s.adminSecurityCheck)
	r.With(permission("settings:write")).Put("/admin/security-check", s.adminUpdateSecurityCheck)
	r.With(permission("settings:write")).Post("/admin/security-check/test", s.adminTestSecurityCheck)
