...
![AppStore v2.11.4 공개 카탈로그 전체 화면](docs/assets/screenshots/captures/today-desktop.webp)

화면 이미지는 비밀이 없는 결정적 fixture로 생성한 v2.11.4 실제 UI의 전체 페이지 WebP 캡처입니다. Desktop·mobile 전체 route의 파일과 SHA-256은 [screenshot manifest](docs/assets/screenshots/manifest.json)에서 검증합니다.
...
```text
Git tag       v2.11.4
Docker image  appstore:v2.11.4
Release file  appstore-v2.11.4.tar.gz
...
```bash
make archive VERSION=v2.11.4
make verify-archive VERSION=v2.11.4
./scripts/smoke-image.sh appstore:v2.11.4
```
...
Pages는 canonical, Open Graph, WebApplication·FAQ JSON-LD, sitemap, robots, `llms.txt`, 키보드 접근성과 반응형 layout을 포함합니다. v2.11.4의 모든 React route를 desktop·mobile로 실제 캡처했으며 [screenshot manifest](docs/assets/screenshots/manifest.json)에 파일별 SHA-256을 기록합니다.
