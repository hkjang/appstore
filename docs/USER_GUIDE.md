...
관리자가 자동 로그인(Silent SSO)을 켠 설치에서는 회사 계정에 이미 로그인돼 있으면 이 화면을 거치지 않고 바로 들어갑니다. 회사 계정 세션이 없으면 "회사 계정 세션이 없어 자동으로 로그인하지 않았습니다"라는 안내와 함께 로그인 화면이 뜨니, 그때는 **회사 계정으로 SSO 로그인**을 누르면 됩니다.
...
| 가이드 문서 | 사용 설명서 파일을 첨부합니다. PDF, Markdown, 텍스트, CSV, Word, PowerPoint, Excel, 한글 문서 · 파일당 20MB · 앱당 10개까지 |
| MCP 지원 · API 지원 | 실제로 제공할 때만 켭니다 |

**문서 첨부**를 누르면 파일을 고르고, 고른 파일은 목록에 *저장 시 업로드됩니다* 로 남아 있다가 **등록**(수정 화면에서는 **변경 저장**)을 누를 때 함께 올라갑니다. 올린 문서는 앱 상세 화면에서 **누구나 로그인 없이** 내려받습니다 — 사내 공개 매뉴얼만 올리세요.

Git 저장소 주소는 수집하지도, 노출하지도 않습니다.

**등록**을 누르면 *승인 Workflow 설정에 따라 즉시 게시되거나 검토 대기 상태가 됩니다.*

조직이 **보안 심의를 필수로** 적용하고 있다면 한 단계가 더 있습니다. 등록한 앱은 바로 검토 대기로 가지 않고 **초안**으로 남고, 내 앱 카드의 **보안 심의** 버튼에서 SecCheck 심의를 마쳐야 제출할 수 있습니다 — 2.4 절을 보세요.

### 2.4. 보안 심의 받기 (필수로 설정된 조직만)

![앱 보안 심의 — SecCheck 심의 설명에 붙여 넣을 연동 정보와 심의 상태](assets/screenshots/captures/my-app-security-desktop.webp)

내 앱에서 **보안 심의**를 누르면 세 단계가 순서대로 있습니다.

1. **SecCheck에 심의 등록** — SecCheck에서 심의를 새로 만들 때 **서비스명을 앱 이름과 똑같이** 적고, 이 화면의 **연동 정보**를 **복사해 심의 설명에 그대로** 붙여 넣습니다. 이 블록이 있어야 어느 앱의 심의인지 확인할 수 있습니다. 붙여 넣은 뒤 다른 줄을 더 쓰는 것은 괜찮지만, 블록 자체는 고치지 마세요.
2. **승인 결과 확인** — SecCheck 심의 주소 끝의 **심의 ID**를 입력하고 **심의 결과 확인**을 누릅니다. AppStore가 SecCheck에 직접 물어봅니다. 아직 승인 전이면 진행 상태만 기록되고, 승인이 끝났으면 **보안 심의 완료**가 됩니다.
3. **등록 제출** — 승인이 확인되면 **검토 제출** 버튼이 열립니다. 누르면 평소처럼 검토 대기 상태가 됩니다.

자주 나오는 안내와 뜻은 이렇습니다.

| 안내 | 뜻과 할 일 |
| --- | --- |
| 심의 설명에 이 앱의 연동 정보가 없습니다 | 연동 정보를 심의 설명에 붙여 넣지 않았거나 내용이 바뀌었습니다. 이 화면에서 다시 복사해 붙여 넣으세요 |
| 심의의 서비스명이 앱 이름과 다릅니다 | SecCheck 심의의 서비스명을 앱 이름과 똑같이 맞추세요 |
| 아직 최종 승인된 심의가 아닙니다 | 승인이 끝난 뒤 다시 확인하세요. 조건부 승인은 사용할 수 없습니다 |
| 심의에 아직 처리되지 않은 항목이 남아 있습니다 | SecCheck에서 미검토 항목과 보완 요청을 정리한 뒤 다시 확인하세요 |
| 앱 정보가 바뀌었습니다 | 앱을 수정하면 연동 정보가 새로 발급됩니다. 다시 복사해 심의에 반영하고 확인하세요 |

**앱을 수정하면 보안 심의 완료 라벨이 사라집니다.** 승인은 그때 심의한 내용에 대한 것이므로, 이름·설명·서비스 URL·태그 같은 정보를 고치면 다시 심의를 받아야 합니다.
...
카드마다 첫 배지가 상태입니다 — **게시됨**, **검토 대기**, **반려**, **초안**, **보관됨**. 공개 카탈로그의 카드에는 이 배지가 없습니다(거기에는 게시된 앱만 있습니다). 게시된 앱의 카드는 공개 카탈로그의 상세 화면으로, 아직 게시되지 않았거나 공개 범위가 비공개인 앱의 카드는 공개 화면이 없으므로 **수정** 화면으로 열리고 **자세히** 버튼이 붙지 않습니다. 몇 개가 게시됐고 몇 개가 검토 대기인지는 **내 홈**(`/my`)의 숫자 카드에서 한 번에 보고, 실제로 공개됐는지는 전체 앱에서 검색해 확인합니다. **반려된 앱만** 카드 아래에 반려 안내와 사유가 함께 표시됩니다 — 3.9 절을 보세요.
...
- 보기 방식: **카드** 또는 **목록**.
- 카드의 **아무 곳이나** 누르면 그 앱의 상세 화면으로 갑니다. 하트(즐겨찾기)와 오른쪽 아래 버튼만 각자의 동작을 합니다.
...
왼쪽에 **앱 소개**와 **태그**가, 오른쪽 **정보** 카드에 버전 · 카테고리 · 언어 · Framework · MCP · API · 담당팀 · 업데이트 시각이 있습니다. 보안 심의를 마친 앱에는 제목 위에 **보안 심의 완료** 라벨이 붙고, 목록 카드에도 같은 라벨이 보입니다. 소개가 길면 앞부분만 보이고 **더 보기**로 펼칩니다 — 그래야 아래의 가이드 문서가 첫 화면에서 밀려나지 않습니다. 등록자가 가이드 문서를 첨부했거나 서비스가 기본 가이드를 싣고 있다면 그 아래 **가이드 문서** 카드가 붙고, 파일 이름·크기·올린 날짜와 함께 **내려받기** 버튼이 나옵니다. 로그인하지 않아도 받을 수 있고, 첨부가 없는 앱에는 카드 자체가 없습니다. **서비스 열기**로 실제 서비스에 접속하고, 옆의 **즐겨찾기** 버튼으로 담아 둡니다. 등록자가 서비스 URL을 아직 넣지 않았다면 버튼 대신 **서비스 URL 준비 중**이 표시됩니다.

### 3.6. 즐겨찾기

![즐겨찾기 — 하트로 담아 둔 앱 모음](assets/screenshots/captures/favorites-desktop.webp)

로그인하지 않아도 쓸 수 있습니다. 로그인 여부와 관계없이 즐겨찾기는 현재 브라우저에 저장되며, 다른 브라우저나 기기로 자동 동기화되지 않습니다. 브라우저 데이터를 삭제하면 즐겨찾기도 함께 사라집니다.
...
가이드 문서도 이 화면에서 관리합니다. 이미 올린 문서에는 **삭제**가, 새로 고른 파일에는 **빼기**가 붙습니다. 둘 다 바로 반영되지 않고 *저장 시 삭제됩니다* · *저장 시 업로드됩니다* 로 표시돼 있다가 **변경 저장**을 누를 때 한 번에 적용됩니다. 삭제를 잘못 눌렀다면 저장하기 전에 **삭제 취소**로 되돌립니다.
...
관리자가 MCP SSO 인증을 켠 설치에서는 이 화면 위에 **키 없이 SSO로 MCP 연결하기** 안내와 MCP 주소(예: `https://apps.example.com/mcp`)가 보입니다. Claude·Cursor 같은 MCP 클라이언트에 그 주소 하나만 넣으면 회사 계정 로그인 창이 열리고, 로그인하면 키 없이 연결됩니다. 이미 회사 계정에 로그인돼 있으면 창이 거의 보이지 않습니다.

- 먼저 **웹으로 한 번 로그인**해 두어야 합니다. 클라이언트가 `이 SSO 계정은 AppStore에 등록되지 않았습니다`를 받으면 브라우저로 로그인한 뒤 다시 연결하세요.
- SSO로 연결하면 관리자가 정한 범위(기본은 조회와 내 앱)만 쓸 수 있고, 내 역할 권한을 넘지 않습니다. 더 넓은 권한이 필요하면 개인 키를 씁니다.
- 로그인 창을 띄울 수 없는 자동화 스크립트는 계속 개인 키를 씁니다. 안내가 보이지 않으면 이 설치는 키만 받는 것입니다.
...
목록은 앱 이름 · 등록자 · 담당팀 · 검토 단계 · 상태 · 요청 시각을 보여 주고, 행 끝의 **열기**로 검토 상세로 들어갑니다.

![검토 상세 — 앱 내용, 가이드 문서, 보안 심의 상태와 이전 검토를 함께 보여 주는 화면](assets/screenshots/captures/review-detail-desktop.webp)

검토 상세는 결정에 필요한 것을 한 화면에 모아 둡니다.

| 영역 | 보이는 것 |
| --- | --- |
| 앱 카드 | 아이콘, 이름, 한 줄 설명, 카테고리·언어·MCP·API·공개 범위 배지 |
| 앱 소개 · 태그 · Screenshot | 등록자가 적은 본문과 태그, 스크린샷 주소 |
| 가이드 문서 | 첨부된 문서를 **내려받기**로 열어 확인합니다 |
| 제출 정보 | 등록자, 담당팀, 검토 단계, 버전, Framework, 최근 수정, slug와 **서비스 URL 열기** |
| 보안 심의 | **보안 심의 완료** 여부, 심의 번호, 승인 시각, **SecCheck 심의 열기** |
| 이전 검토 | 같은 앱의 지난 결정과 그때 남긴 의견(최대 20건) |

결정은 아래 **검토 결정** 카드에서 합니다.

- **검토 의견** — 승인할 때는 선택이고, **반려할 때는 필수**입니다. 2,000자까지 쓸 수 있고 줄바꿈이 그대로 유지됩니다. 여기 적은 글은 등록자의 **내 앱**과 **앱 수정** 화면에 그대로 보이고, 다음 단계 검토자의 **이전 검토**에도 남습니다.
- **승인 및 게시** — 의견을 함께 저장하며 승인합니다. 다단계 검토라면 다음 단계로 넘어가고, 마지막 단계면 게시됩니다.
- **반려** — 의견을 비워 두면 *반려하려면 사유를 입력하세요.* 라고 알려 주고 보내지 않습니다.

이미 처리된 검토를 열면 누가 언제 어떤 결정을 했는지 안내가 뜨고 두 버튼은 잠깁니다.
...
5. `Authorization` 헤더에 넣어 `/api/v1` 또는 `/mcp`를 호출합니다. 사람이 직접 쓰는 MCP 클라이언트라면 키 대신 3.10의 *키 없이 SSO로 MCP 연결하기*가 더 간단합니다.
6. 만료가 다가오면 **회전** → 클라이언트 교체 → 옛 키 **폐기** 순서로 무중단 교체합니다.

### 4.4. 자주 쓰는 앱 모아 두기

앱 카드나 앱 상세의 하트를 누르면 **즐겨찾기**에 담깁니다. 사이드바의 즐겨찾기에서 한 번에 봅니다. 로그인 여부와 관계없이 즐겨찾기는 현재 브라우저에 저장되며, 다른 브라우저나 기기로 자동 동기화되지 않습니다. 브라우저 데이터를 삭제하면 즐겨찾기도 함께 사라집니다.
