...
왼쪽에 **앱 소개**와 **태그**가, 오른쪽 **정보** 카드에 버전 · 카테고리 · 언어 · Framework · MCP · API · 담당팀 · 업데이트 시각이 있습니다. 등록자가 가이드 문서를 첨부했거나 서비스가 기본 가이드를 싣고 있다면 그 아래 **가이드 문서** 카드가 붙고, 파일 이름·크기·올린 날짜와 함께 **내려받기** 버튼이 나옵니다. 로그인하지 않아도 받을 수 있고, 첨부가 없는 앱에는 카드 자체가 없습니다. **서비스 열기**로 실제 서비스에 접속하고, 옆의 **즐겨찾기** 버튼으로 담아 둡니다. 등록자가 서비스 URL을 아직 넣지 않았다면 버튼 대신 **서비스 URL 준비 중**이 표시됩니다.
