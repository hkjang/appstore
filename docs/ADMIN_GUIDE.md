...
![메일 알림 — SMTP 릴레이 설정, 보낼 이벤트, 시험 발송과 발송 기록](assets/screenshots/captures/admin-mail-desktop.webp)

관리자 → **메일 알림**에서 사내 SMTP 릴레이로 이벤트 알림을 보냅니다. 설정은 PostgreSQL(`system_settings`의 `mail.*` 행)에 저장되고 변경은 감사 로그에 `mail.setting.update`로 남습니다. **기본값은 꺼짐**입니다. 새로 설치한 곳은 이 화면을 손대기 전까지 아무것도 보내지 않고 기록도 남기지 않습니다.

| 키 | 화면 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `mail.enabled` | 메일 알림 사용 | `false` | 켜려면 릴레이 주소와 보내는 주소가 있어야 합니다 |
| `mail.smtp_host` | SMTP 릴레이 주소 | — | 사내 릴레이 호스트. 폐쇄망에서는 `postra`를 권장합니다 |
| `mail.smtp_port` | 포트 | `25` | 사내 릴레이는 대개 25 |
| `mail.security` | 보안 | `auto` | `auto` · `none` · `starttls` · `tls`. `auto`는 서버가 STARTTLS를 알리면 쓰고 아니면 평문. 465는 자동으로 `tls` |
| `mail.skip_tls_verify` | TLS 인증서 검증 생략 | `false` | 사내 인증서가 사설일 때만 |
| `mail.username` · `mail.password` | 사용자 이름 · 비밀번호 | 빈 값 | **선택 사항.** 비우면 인증하지 않습니다. 비밀번호는 `ENCRYPTION_KEY`로 암호화되어 저장되고 조회 API가 돌려주지 않습니다 — 화면에는 "설정됨"만 보입니다 |
| `mail.from_address` · `mail.from_name` | 보내는 주소 · 이름 | — · `AppStore` | 메일의 From |
| `mail.base_url` | 메일 속 링크의 기준 주소 | — | 비우면 시스템 설정의 서비스 접속 URL |
| `mail.timeout_seconds` | 제한 시간 | `10` | 연결·세션 전체의 한도(1~120초) |
| `mail.notify_review_requested` | 검토 요청 | `true` | 앱이 검토 대기에 들어가면 그 단계를 처리할 수 있는 검토자 역할 전원과 같은 팀의 팀장에게 |
| `mail.notify_review_decided` | 검토 결과 | `true` | 승인(마지막 단계)이나 반려가 나면 등록한 사람에게. 중간 단계 승인은 보내지 않습니다 |
| `mail.notify_app_status` | 관리자 상태 변경 | `true` | 관리자가 다른 사람의 앱 게시 상태를 바꾸면 소유자에게 |

**동작.** 메일은 요청을 막지 않습니다 — 검토를 승인하는 요청은 릴레이가 느리든 죽어 있든 즉시 끝나고, 발송은 배경에서 한 번 더 시도(2초 뒤)한 뒤 결과를 기록합니다. 자기가 한 일은 자기에게 보내지 않으며(검토자가 곧 등록자면 조용합니다), 이메일이 없는 계정은 건너뜁니다. 수신자 명부는 따로 없고 사용자 표의 이메일을 그대로 씁니다 — SSO에서 이메일이 오지 않는 사용자는 받지 못하므로 사용자 화면에서 채웁니다.

**시험 발송.** 설정을 저장한 뒤 *시험 발송 받는 사람*에 주소를 넣고(비우면 내 계정 이메일) **시험 발송**을 누르면 저장된 설정으로 실제 한 통을 보내고 릴레이의 답을 그 자리에서 보여 줍니다. 꺼져 있으면 409, 릴레이가 거절하거나 닿지 않으면 502와 함께 원인이 옵니다.

**발송 기록.** 같은 화면 아래에 시도마다 한 줄 — 시각, 이벤트, 받는 사람, 제목, 결과(성공·실패·대기)와 실패 원인. **본문은 담지 않습니다.** "메일이 안 왔다"는 문의는 여기서 받는 사람으로 찾습니다: 줄이 없으면 이벤트 스위치가 꺼졌거나 그 사람에게 이메일이 없는 것이고, 실패면 원인이 적혀 있습니다. API로는 `GET /api/v1/admin/mail/deliveries?status=failed`입니다.
...
| 시험 발송이 502 `SMTP 연결 실패` | 관리자 → 메일 알림, 컨테이너에서 릴레이 포트 도달 여부 | 릴레이 주소·포트와 방화벽을 확인합니다. `서버가 인증을 지원하지 않습니다`면 사용자 이름을 비웁니다 |
| 검토 요청 메일이 오지 않는다 | 관리자 → 메일 알림 → **발송 기록** | 줄이 없으면 이벤트 스위치나 검토자의 이메일(사용자 화면)을, 실패면 적힌 원인을 봅니다. 등록자 본인에게는 보내지 않습니다 |
...
| 메일 알림 | 꺼짐 | 사내 릴레이(`postra`)를 가리켜 알림이 밖으로 나가지 않게 합니다 |
...
- OIDC Client Secret, AI API Key, SMTP 비밀번호는 `ENCRYPTION_KEY`로 인증 암호화되어 DB에 저장되고, **조회 API가 없습니다.** 변경만 가능하며 로그에도 남지 않습니다.
