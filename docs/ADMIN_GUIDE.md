...
대상 버전: **v2.11.5** · 실린 화면은 모두 이 버전을 실제로 띄워 찍은 것입니다.
...
릴리스 자산 하나(`appstore-vX.Y.Z.tar.gz`)로 폐쇄망까지 반입할 수 있습니다. 아래는 v2.11.5 기준입니다.
...
```bash
sha256sum appstore-v2.11.5.tar.gz          # 릴리스 노트의 SHA-256과 대조
gzip -t appstore-v2.11.5.tar.gz
gzip -dc appstore-v2.11.5.tar.gz | docker load
docker image inspect appstore:v2.11.5 \
...
```bash
APPSTORE_VERSION=v2.11.5 docker compose up -d --no-build
docker compose ps
...
첫 기동에서 서비스는 순서대로 마이그레이션 적용 → `ENCRYPTION_KEY` 지문 확인 → 초기 카탈로그 seed → Bootstrap 관리자 생성 → 내장 가이드 첨부를 수행합니다. `/health/ready`가 200을 돌려주면 여기까지 끝난 상태입니다. 마지막 단계는 로그에 `bundled guides synced`로 남고 `apps`(짝이 맞은 앱 수)·`attached`·`replaced`·`unchanged`·`unmatched`(같은 slug의 앱이 아직 없는 가이드)·`failed` 수를 함께 적습니다.
...
- **내장 가이드** — 이미지에는 사내 서비스별 `USER_GUIDE.pdf`·`ADMIN_GUIDE.pdf`가 실려 있고, 기동할 때마다 **디렉터리 이름과 slug를 각각 소문자로 바꿔 같은 앱**에만 *사용자 가이드*·*관리자 가이드*로 첨부됩니다. 같은 파일 이름의 문서가 이미 있으면 내용이 바뀐 것만 제자리에서 교체하고(문서 ID와 내려받기 주소는 그대로), 같으면 손대지 않으며, 다른 이름으로 올린 문서는 건드리지 않습니다. 앱이 이미 10개를 다 채웠거나 아직 등록되지 않은 slug는 건너뛰고 로그에 남깁니다. 자동으로 붙은 문서는 올린 사람이 비어 있으며, 지워도 다음 기동에서 다시 붙습니다. 내장 가이드를 갱신하는 방법은 README의 `make guides`를 참고하세요.
...
5. 새 이미지가 기동하면서 그 릴리스에 실린 서비스 가이드를 slug가 같은 앱에 자동 첨부합니다. 로그의 `bundled guides synced` 줄에서 `failed`가 0인지 확인합니다.
6. `/health/live`, `/health/ready`, `/api/version`이 새 버전인지 확인하고, 익명 탐색 → 로그인 → `/admin/users` 새로고침 → 앱 등록·검토 → 키 발급·회전·폐기 → AI 스트리밍 순으로 실제 화면을 확인합니다.
...
docker rename appstore-v2.11.4-stopped appstore
